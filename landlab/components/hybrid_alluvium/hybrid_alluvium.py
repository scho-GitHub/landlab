...
from landlab.components.hybrid_alluvium.hybrid_global_solver import hybrid_H_etab_Qs_solver
    ...
    def run_one_step(self, 
        ...
        if global_solve:
           ...
           # need to permit some nodes to be the boundary condition.            
            ...
            self.v0 = np.concatenate((s_d, b_e, qs), axis=0)
            #distinguish cases:
            
            # solve using fsolve
            self.solver_output = root(hybrid_H_etab_Qs_solver,
                                      self.v0,
                                      args = (self.soil__depth.copy(), 
                                              self.bedrock__elevation.copy(), 
                                              self.flow_receivers, 
                                              self.lil_q, 
                                              self.q, 
                                              self.K_sed, 
